    ...
    while True:
        ...
        try:
            try:
                ...
                disp_string_convCur_value = "1" + c1 + ":" + \
                    str(round(
                        api_ExchangeRateAPI_json['conversion_rate'], 2)) + c2
                print(str(datetime.now()) + " " +
                      "thr3_dollarconv got an update from ERA: " + disp_string_convCur_value)
                time.sleep(86400)
                ...
                print(str(datetime.now()) + " " +
                      "thr3_dollarconv got an update from FCC: " + disp_string_convCur_value)
                time.sleep(3600)
        except KeyError:
...
def thread_get_weather_info(tokenOWM=api_OpenWeather_token, cityid=api_OpenWeather_yourCity):
    ...
    while True:
        try:
            ...
            disp_string_weatherInfo = str(round(api_OpenWeather_json['main']['temp'])) + "￟C - " + \
...
def first_line():
    ...
    my_ip = get_ip()
    display.lcd_display_string("i:" + my_ip[-3:] + " " + str(mytime.tm_mday).zfill(2) + str(
        mytime.tm_mon).zfill(2) + " " + str(mytime.tm_hour) + ":" + str(mytime.tm_min).zfill(2), 1)
...
if __name__ == "__main__":
    ...
    thr1_get_quotable = threading.Thread(
        target=thread_get_quotable, daemon=True)
    thr2_currconv = threading.Thread(
        target=thread_get_currency_conversion, daemon=True)
    thr3_weatherinfo = threading.Thread(
        target=thread_get_weather_info, daemon=True)
